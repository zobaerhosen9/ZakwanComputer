...
// Default Seed Data Constants for both database seeding and client-side fallback/demo mode
...
export function runWithTimeout<T>(promise: Promise<T>, ms: number, errorMsg = "Timeout"): Promise<T> {
...
export async function seedInitialDataIfEmpty() {
  try {
    // 5-second timeout on checking empty database
    const pSnap = await runWithTimeout(getDocs(productsRef), 5000, "Database connection timeout during check");
    if (!pSnap.empty) {
    ...
    // 2. Default Customers
    DEFAULT_CUSTOMERS.forEach((cust) => {
    ...
    // 5-second timeout on write transaction commit
    await runWithTimeout(batch.commit(), 5000, "Database transaction timed out during seeding");
    console.log("Database seeded successfully!");
  } catch (error) {
    console.error("Error seeding initial data: ", error);
    throw error; // Propagate to caller so it can switch to Demo Mode or show connection error
  }
}
